// If you want to build a "Free-Look" or "FPS-style" camera where moving your mouse lets you look up, down, left, and right, you have to use Euler angles.
// a 3D direction vector like (0.0f, 0.0f, -1.0f) is great for computers, but it is terrible for human input. If I tell you to "look 45 degrees to the right and 15 degrees up," translating that directly into a 3D vector in your head is almost impossible. Euler angles bridge that gap.
// The Three Angles (Pitch, Yaw, and Roll)
// Euler angles define any 3D rotation using three independent, human-readable angles.
// Pitch: Looking up and down. Mathematically, this is rotation around the X-axis. (Nodding your head "yes").
// Yaw: Looking left and right. Mathematically, this is rotation around the Y-axis. (Shaking your head "no").
// Roll: Tilting your head to the side. Mathematically, this is rotation around the Z-axis. (Doing a barrel roll).

// For a standard 3D camera, we almost entirely ignore Roll.

// The Y Component (Up/Down): This is purely controlled by Pitch. y=sin(pitch)
// The X and Z Components (Left/Right/Depth): These are controlled by both Yaw and Pitch. First, Yaw dictates where on the horizontal circle you are pointing using sine and cosine. Second, if you look straight up into the sky (Pitch = 90 -> sin(90) = 1), your horizontal X and Z distances shrink to 0. Therefore, X and Z must be multiplied by the cosine of the Pitch.
// x=cos(yaw)⋅cos(pitch)
// z=sin(yaw)⋅cos(pitch)
...
// Mouse Input Globals
float g_yaw   = -90.0f; // Start pointing down the -Z axis
float g_pitch =  0.0f;
float g_lastX =  400.0f; // Center of your 800x800 window
float g_lastY =  400.0f;
bool g_firstMouse = true;
float g_fov = 45.0f; // Default standard FOV(Field of View) ; Zoom Global
...
void windowInitialize()
{
    ...
    glfwSetInputMode(g_window, GLFW_CURSOR, GLFW_CURSOR_DISABLED); // Hides and captures the cursor
...
void mouse_callback(GLFWwindow* window, double xposIn, double yposIn)
{
    ...
    // Prevent massive screen jump on the very first frame
    if(g_firstMouse)
    {
        g_lastX = xpos;
        g_lastY = ypos;
    ...
    // Apply sensitivity so it doesn't spin wildly out of control
    ...
    // The Gimbal Lock Safety Net: Cap the pitch
    if (g_pitch > 89.0f)
        g_pitch = 89.0f;
    if (g_pitch < -89.0f)
        g_pitch = -89.0f;
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
    // Subtract the scroll movement from our FOV
    g_fov -= (float)yoffset;
    
    // Clamp the limits so we don't break the math
    if (g_fov < 1.0f)
        g_fov = 1.0f;   // Maximum zoom in
    if (g_fov > 45.0f)
        g_fov = 45.0f;  // Maximum zoom out (back to default)
}
...
void mainRenderingLoop()
{
    ...
    while(!glfwWindowShouldClose(g_window))
    {
        ...
        view_matrix = glm::lookAt(g_cameraPos, g_cameraPos + g_cameraFront, g_cameraUp); // The glm::LookAt function requires a position, target and up vector respectively.
        ...
        // PROJECTION: Use the dynamic g_fov variable!
        glm::mat4 projection_matrix = glm::perspective(glm::radians(g_fov), 800.0f / 800.0f, 0.1f, 100.0f);
